...
$(function(){
   ...
   $("#nickCheck").click(function(){
      if(    $("#nick").val().length < 2   ){
         //alert("아이디는 4글자 이상이어야 합니다.");
         $("#checkResult").text("닉네임은는 2글자 이상이어야 합니다.");
         $("#joinBtn").attr("disabled", true);//비활성화하기
      } else {
         //alert("서버와 통신을 시작합니다.");
         //$("#checkResult").text("서버와 통신을 시작합니다.");
         $.ajax({
            ...
            if(result2 == 1){
               ...
               $("#checkResult2").text($("#nick").val()+ "(은)는 이미 가입된 닉네임입니다. 다른 닉네임를 입력하세요.");
               $("#nick").val("");
               $("#joinBtn").attr("disabled", true);//비활성화하기
            }else{
               //alert("가입할 수 있는 ID입니다.");
               ...
               $("#checkResult2").text("사용할 수 있는 닉네임입니다.");
               $("#joinBtn").attr("disabled", false);//활성화 하기
            }
         }).fail(function(xhr, status, errorThrown){
            alert("문제가 발생했습니다. 다시 시도하세요");
   ...
   $("#joinBtn").click(function(){
      ...
      if(pw == pw2){
         ...
         var form = $('<form></form>');
         form.attr("name","id_form");
         form.attr("method","post");
         form.attr("action","./membership");
         
         
         form.append(   $('<input>', {'type':'hidden', 'name':'id', 'value':id})   );
         form.append(   $('<input>', {'type':'hidden', 'name':'pw', 'value':pw  })   );
         form.append(   $('<input>', {'type':'hidden', 'name':'name', 'value':name })   );
         form.append(   $('<input>', {'type':'hidden', 'name':'nick', 'value':nick })   );
...
<body>
   ...
   <%@ include file="header.jsp" %>
    ...
    <div id="contents" class="inner">
        <h2><a href="javascript:void(0);"><div class="logo_wrap"><img src="images/logo.png" alt="logo"></div></a></h2>
        <div id="cont0">
        <ul id="cont1">
            <li>
                <div class="agree">
...
(시행일) 본 약관은 2018년 8월 1일부터 시행됩니다. 개정된 약관의 적용일자 이전 이용자 또는 회원은 개정된 이용약관의 적용을 받습니다.
                ...
                <div id="agreeBt" style="margin-bottom: 10px;" ><input type="checkbox" name="agree" id="agree2"> <label for="agree2"> 동의합니다.</label></div>
                </li>       
            <li>
                <div class="agree">
                <h3>개인정보처리방침</h3>
    ...
    - 행정자치부
      ...
      · 개인정보 보유 및 이용 기간 : 행정자치부에서는 이미 보유하고 있는 개인정보이기 때문에 별도로 저장하지 않음
        ...
        <ul id="cont2">
            <li>
                <h3>아이디</h3>
                <input type="text"  id="id"  placeholder="아이디를 입력해주세요" required="required">
                <button type="submit"  id="idCheck">중복확인</button>
                <p id="checkResult"></p>
             </li>
            <li><h3>비밀번호</h3><input type="password" id="pw" placeholder="암호를 입력해주세요" required="required"></li>
            <li><h3>비밀번호확인</h3><input type="password" id="pw2" placeholder="암호를 다시 입력해주세요" required="required"></li>
            <li><h3>이름</h3><input type="text" id="name" placeholder="이름을 입력해주세요" required="required" ></li>
            <li>
               <h3>닉네임</h3>
               <input type="text" id="nick" placeholder="닉네임을 입력해주세요" required="required">
               <button type="submit" id="nickCheck">중복확인</button>
               <p id="checkResult2"></p>
            </li>
            <li>
               <h3>생년월일</h3>
               <input type="text" id="birthday" placeholder="생년월일 6자리를 입력해주세요" required="required">
            </li>
            
            <li>
               <h3>성별</h3>
                 <input type="text" id="sex" placeholder="성별을 입력해주세요" required="required">
            <li>
                <h3>이메일</h3>
                <input type="text"  id="Email" placeholder="이메일을 입력해주세요" required="required">
            </li>
            <li>  
                <h3>휴대전화</h3>
            ...
            <li class="last">
               <button type="button" id="joinBtn" disabled="disabled" >가입하기</button>
